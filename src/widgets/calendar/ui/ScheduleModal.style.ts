...
export const MemberDropdownButton = styled.button<{ $hasValue: boolean }>`
  ${fieldBoxStyle}
  ${token.flexBetween}
  height: ${FIELD_HEIGHT};
  padding: 10px 12px;
...
export const MemberDropdownIcon = styled.img<{ $isOpen: boolean }>`
  width: 9px;
  height: 16px;
  flex: 0 0 auto;
  cursor: pointer;
  transform: ${({ $isOpen }) => `rotate(${$isOpen ? '90deg' : '-90deg'})`};
`

export const MemberDropdownMenu = styled.div`
  ${token.flexColumnStart}
  position: absolute;
  z-index: 2;
  top: calc(100% + 8px);
...
export const MemberGenerationButton = styled.button<{ $selected: boolean }>`
  ${memberSelectButtonStyle}
  background: ${({ $selected }) =>
...
export const MemberItemButton = styled.button<{ $selected: boolean }>`
  ${memberSelectButtonStyle}
  width: 100%;
  background: ${({ $selected }) =>
