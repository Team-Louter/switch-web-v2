...
const BlockFallback = lazy(() =>
  import('./CommunityPostBlockFallback').then(
    ({ CommunityPostBlockFallback }) => ({
      default: CommunityPostBlockFallback,
    }),
  ),
);
...
function imageUrl(block: Block, files: readonly PostFileResponse[]) {
  ...
  const url =
    files.find((file) => file.fileName === block.props.url)?.fileUrl ??
    block.props.url;
...
export function CommunityPostBlockContent({
  ...
  for (const block of blocks) {
    ...
    if (
      !supportsStaticBlock(block) &&
      previous &&
      !supportsStaticBlock(previous[0])
    ) {
      previous.push(block);
    } else {
      groups.push([block]);
  ...
  return (
    <S.Content aria-label="게시글 본문">
      {groups.map((group) => {
        ...
        if (href)
          return <LinkPreview key={`${block.id}:${href}`} href={href} />;
