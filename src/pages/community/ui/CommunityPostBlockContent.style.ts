...
export const Figure = styled.figure<{ $width?: number }>`
  width: ${({ $width }) => $width === undefined ? '100%' : `min(100%, ${$width}px)`};
  margin: 0;
  padding: 3px 0;
...
export const ImageSurface = styled.div<{ $isLoading: boolean }>`
  ...
  background: ${({ $isLoading }) => $isLoading ? token.colors.gray.gray10 : 'transparent'};
  ...
  &::after {
    ...
    background: linear-gradient(90deg, transparent 25%, ${token.colors.gray.gray0} 50%, transparent 75%);
...
export const LinkDetails = styled.a`
  ...
  &:hover { background: ${token.colors.gray.gray10}; }
